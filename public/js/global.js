			   	...
			   	if(respuesta.facturas != null){
					...
					for (var j in respuesta.facturas) {
						...
						var row = '<tr id="facturaRow' + respuesta.facturas[j]['id'] + '"' + 'class="factura">\
					                ...
					                <td>$ ' + total  + '</td>\
					                <td>' + respuesta.facturas[j]['pisapesos']  + '</td>\
					                <td><a class="btn btn-primary" style="width: 30%; background: #2a91d6;" href="'+  window.location.href.replace('/home', '') +'/factura_info/' + respuesta.facturas[j]['id']  + '">ver</a> ' + respuesta.facturas[j]['fecha']  + '</td>\
					               </tr>';
					             $('#factura tr:last').after(row);
...
function editar_registro(id_registro, descripcion){
	...
	$('#editar').click(function(){
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: {'id_registro' : id_registro, 'descripcion' :  descripcion},
				beforeSend: function(){
                    $('.before').append('<img align="center" src="./img/preloader/preloader.gif"/>');
...
function cargar_datatable(){
		...
		$('#a_validar').dataTable();
	      ...
	      tablas = $('.table-factura, .table-no-aprobados, .table-aprobados, .table-pendientes').DataTable({
	         dom: 'lBfrtip',
	         "iDisplayLength": 5,
